...
export default function LocaleHomePage({ params }) {
  ...
  // Asegurar que el contexto de idioma coincida con la URL /[locale]/
  useEffect(() => {
    if (locale && (locale === 'es' || locale === 'en') && language !== locale) {
      setLanguage(locale);
    }
  }, [locale]);
  ...
  function filteredToursList() {
    ...
    if (filtroPais !== 'Todos') {
      list = list.filter(tour => {
        const p = (tour.pais || '').toLowerCase();
        const f = filtroPais.toLowerCase();
        return p === f || (f === 'peru' && p === 'perú') || (f === 'perú' && p === 'peru');
      });
    }
    if (filtroCategoria !== '*') {
    ...
    if (busqueda.trim() !== '') {
      ...
      list = list.filter(tour =>
        (tour.nombre && tour.nombre.toLowerCase().includes(term)) ||
        (tour.nombre_en && tour.nombre_en.toLowerCase().includes(term)) ||
        (tour.descripcion && tour.descripcion.toLowerCase().includes(term)) ||
        (tour.descripcion_en && tour.descripcion_en.toLowerCase().includes(term)) ||
        (tour.pais && tour.pais.toLowerCase().includes(term)) ||
        (tour.ciudad && tour.ciudad.toLowerCase().includes(term)) ||
        (tour.categoria && tour.categoria.toLowerCase().includes(term))
      );
    }
    return list;
  }

  // Lista de categorías con traducción
  const categoriesList = [
    { key: '*', label: t('catalog.categories.all') || t('catalog.category_all') },
    { key: 'Full Days', label: t('catalog.categories.full_days') || 'Full Days' },
    { key: 'Trekking', label: t('catalog.categories.trekking') || 'Trekking' },
    { key: 'Trek & Climb', label: t('catalog.categories.trek_climb') || 'Trek & Climb' },
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--background)] relative">
      ...
      <section id="tours" className="w-full border-t border-[var(--border)]/20 bg-[var(--sidebar)]/60 relative z-10 scroll-mt-14">
          ...
          <div className="p-4 border-b border-[var(--border)] bg-[var(--card)]/95 backdrop-blur-md space-y-3 sticky top-[52px] z-10">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-black text-[var(--foreground)] flex items-center gap-1.5">
                ...
                {t('catalog.title_country').replace('{country}', filtroPais === 'Todos' ? (t('catalog.countries.all') || t('footer.sudamerica')) : (t(`catalog.countries.${filtroPais.toLowerCase()}`) || filtroPais))}
              </h2>
            ...
            <div className="flex flex-nowrap gap-1.5 overflow-x-auto no-scrollbar pb-0.5">
              {categoriesList.map((cat) => {
                const isSelected = filtroCategoria === cat.key;
                return (
                  <button
                    key={cat.key}
                    onClick={() => { setFiltroCategoria(cat.key); setVisibleCount(6); }}
            ...
            {filteredTours.length === 0 ? (
              <div className="py-16 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <p className="text-xs text-[var(--muted-foreground)]/80 max-w-xs leading-relaxed">
                  {t('catalog.no_tours_desc').replace('{country}', filtroPais === 'Todos' ? (t('catalog.countries.all') || 'Sudamérica') : (t(`catalog.countries.${filtroPais.toLowerCase()}`) || filtroPais))}
                </p>
              </div>
            ) : (
              <>
      ...
      <section id="contacto" className="bg-[var(--sidebar)] border-t border-[var(--border)]/50 py-16 px-6 relative z-10 scroll-mt-14">
